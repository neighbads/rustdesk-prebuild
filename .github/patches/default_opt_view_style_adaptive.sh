...
echo "Applying default_opt_view_style_adaptive patch..."

# Check if OPTION_VIEW_STYLE constant exists
if ! grep -q 'keys::OPTION_VIEW_STYLE' "$CONFIG_FILE"; then
    echo "✗ OPTION_VIEW_STYLE constant not found in keys module"
    exit 1
fi

# Find and patch the UserDefaultConfig get method for non-mobile platforms
...
echo "Default opt view style adaptive patch completed"
